...
import com.poweruniverse.oim.activiti.MyExpressionManager;
...
public class BpmnParse extends Parse {
  ...
  public TaskDefinition parseTaskDefinition(Element taskElement, String taskDefinitionKey, ProcessDefinitionEntity processDefinition) {
    ...
    //根据gongNengCZDM 确定task的执行人(潜在执行人)
    //wangyang 2012-09-15
    String candidateUserAuthId = taskElement.attribute("http://www.oim.com:gongNengCZDM");
    if (candidateUserAuthId != null) {
    	taskDefinition.setAssigneeExpression(null);
    	taskDefinition.getCandidateGroupIdExpressions().clear();
        taskDefinition.addCandidateUserIdExpression(((MyExpressionManager)expressionManager).createTaskCandidateExpression(candidateUserAuthId));
    }
 ...
 ActivityImpl activity = createActivityOnScope(transactionElement, scope);
  ...
  public void parseSequenceFlow(Element processElement, ScopeImpl scope) {
    for (Element sequenceFlowElement : processElement.elements("sequenceFlow")) {
      ...
      if (sourceActivity == null) {
        ...
        //取得颜色定义  wangyang 2012-11-05
        String linecolor = sequenceFlowElement.attribute("http://www.oim.com:linecolor");
        transition.setProperty("linecolor",linecolor);
  ...
  public void parseSequenceFlowConditionExpression(Element seqFlowElement, TransitionImpl seqFlow) {
    ...
    if (conditionExprElement != null) {
      ...
      Condition expressionCondition = new UelExpressionCondition(((MyExpressionManager)expressionManager).createSequenceFlowExpression(expression,((ProcessDefinitionEntity)seqFlow.getProcessDefinition()).getKey(),seqFlow.getId()));
