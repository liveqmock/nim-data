...
public class ExecutionEntity extends VariableScopeImpl implements ActivityExecution, ExecutionListenerExecution, Execution, PvmExecution, ProcessInstance, InterpretableExecution, PersistentObject {
  ...
  private static org.apache.log4j.Logger oim_Workflow_logger =  org.apache.log4j.Logger.getLogger("oim_Workflow");
  ...
  public void take(PvmTransition transition) {
	oim_Workflow_logger.debug("   task transition 开始...");
    if (this.transition!=null) {
    	oim_Workflow_logger.error("      error:already taking a transition");
    	throw new PvmException("already taking a transition");
    }
    if (transition==null) {
    	oim_Workflow_logger.error("      error:transition is null");
      throw new PvmException("transition is null");
    }
    setActivity((ActivityImpl)transition.getSource());
    setTransition((TransitionImpl) transition);
    
    LiuChengJS liuChengJS = (LiuChengJS)this.getVariable("liuChengJS");
    if(liuChengJS==null){
    	oim_Workflow_logger.error("      LiuChengJS...null");
    	//记录路径 以及流程检视
    	Integer dataId = null;
    	String businessKey =this.getProcessBusinessKey();
    	if(businessKey!=null){
        	oim_Workflow_logger.debug("      task businessKey..."+businessKey);
        	dataId = Integer.valueOf(businessKey);
    		//流程用户
    		Integer historyId = null;
    		String dengLuDH = null;
    		String yongHuMC = null;
    		if(this.hasVariable("yongHuDM") ){
    			oim_Workflow_logger.debug("      task yongHuDM..."+this.getVariable("yongHuDM"));
    			Boolean isInstanceStart = (Boolean)this.getVariable("isInstanceStart");
            	...
            	try {
            		...
            		if(gongNengGZLDH.indexOf("-")>0){
            			gongNengDH = gongNengGZLDH.substring(0,gongNengGZLDH.indexOf("-"));
            		}else{
            			gongNengDH = gongNengGZLDH;
            		}
            		oim_Workflow_logger.debug("      task gongNengDH..."+gongNengDH);
//    				ServiceUtils.saveLiuChengLJ(gongNengDH,dataId,transition.getId());
    				String sourceType = ""+transition.getSource().getProperty("type");
    				//源节点对应的流程检视应该已经存在了
    				LiuChengJS parentLCJS = (LiuChengJS)sess.createCriteria(LiuChengJS.class)
    							.add(Restrictions.eq("gongNengDH",gongNengDH))
    							.add(Restrictions.eq("gongNengObjId",dataId))
    							.add(Restrictions.eq("shiFouSC", false))
    							.add(Restrictions.eq("caoZuoDH", transition.getSource().getId()))
    							.addOrder(Order.desc("chuangJianRQ"))
    							.setMaxResults(1)
    							.uniqueResult();
    				oim_Workflow_logger.debug("      task parentLCJS..."+parentLCJS+",sourceType:"+sourceType);
    				if("parallelGateway".equals(sourceType)){
    				}else if("userTask".equals(sourceType)){
    				}else if ("startEvent".equals(sourceType)){
    					parentLCJS = new LiuChengJS();
    					parentLCJS.setGongNengDH(gongNengDH);
    					parentLCJS.setGongNengObjId(dataId);
						parentLCJS.setCaoZuoDH("Start");
						parentLCJS.setCaoZuoMC("启动");
						parentLCJS.setCaoZuoRen(yongHuMC+" ");
						parentLCJS.setShiFouGDJD(true);
						parentLCJS.setShiFouCL(true);
						parentLCJS.setShiFouWC(true);
						parentLCJS.setChuangJianRQ(Calendar.getInstance().getTime());
						sess.save(parentLCJS);
    				}
    				
    				//创建目标节点的流程检视
    				LiuChengJS childLCJS = null;
    				String destinationType = ""+transition.getDestination().getProperty("type");
    				String destinationCandidateUserString = "";
    				oim_Workflow_logger.debug("      task destinationType:"+destinationType);
    				if("parallelGateway".equals(destinationType)){
    					//并行节点有可能已创建了流程检视
    					//如果重新走流程是不是有可能错误啊！！！
    					childLCJS = (LiuChengJS)sess.createCriteria(LiuChengJS.class)
    							.add(Restrictions.eq("gongNengDH",gongNengDH))
    							.add(Restrictions.eq("gongNengObjId",dataId))
    							.add(Restrictions.eq("caoZuoDH", transition.getDestination().getId()))
    							.add(Restrictions.eq("shiFouWC", false))
    							.add(Restrictions.eq("shiFouSC", false))
//    							.addOrder(Order.asc("shiFouWC"))
    							.addOrder(Order.desc("chuangJianRQ"))
    							.setMaxResults(1)
    							.uniqueResult();
    					if(childLCJS==null){
    						childLCJS = new LiuChengJS();
    						childLCJS.setGongNengDH(gongNengDH);
    						childLCJS.setGongNengObjId(dataId);
    						childLCJS.setCaoZuoDH(transition.getDestination().getId());
    						childLCJS.setCaoZuoRen(yongHuMC+" ");
    						childLCJS.setCaoZuoMC("并行");
    						childLCJS.setShiFouGDJD(true);
    						childLCJS.setShiFouCL(false);
    						childLCJS.setShiFouWC(false);
    						childLCJS.setChuangJianRQ(Calendar.getInstance().getTime());
    					}
    				}else if("userTask".equals(destinationType)){
    					String currentCaoZuoDH = transition.getDestination().getId();
    					childLCJS = new LiuChengJS();
    					childLCJS.setGongNengDH(gongNengDH);
    					childLCJS.setGongNengObjId(dataId);
    					childLCJS.setCaoZuoDH(currentCaoZuoDH);
    					childLCJS.setCaoZuoMC((String)transition.getDestination().getProperty("name"));
    					childLCJS.setShiFouCL(false);
    					childLCJS.setShiFouWC(false);
    					childLCJS.setChuangJianRQ(Calendar.getInstance().getTime());
    					
    					oim_Workflow_logger.debug("      task childLCJS:"+childLCJS);
    					...
    					List<YongHu> candidateUsers = TaskUtils.getCandidateUsers(gongNengCZDM, dataId,isInstanceStart,null);
    					...
    					oim_Workflow_logger.debug("      task destinationCandidateUserString:"+destinationCandidateUserString);
    					//非启动流程的情况下
    					GongNeng gn = GongNeng.getGongNengByDH(gongNengDH);
    					
    					//查询当前功能-工作流记录
    					GongNengGZL gngzl = (GongNengGZL)sess.createCriteria(GongNengGZL.class)
    						.add(Restrictions.eq("gongNeng.id", gn.getGongNengDM()))
    						.add(Restrictions.eq("gongNengGZLBH", gongNengGZLDH.substring(gongNengGZLDH.indexOf("-") +1)))
    						.uniqueResult();
    					String startCZDH = null;
    					if(gngzl!=null && gngzl.getStartGNCZ()!=null){
    						startCZDH = gngzl.getStartGNCZ().getCaoZuoDH();
    					}
    					
    					oim_Workflow_logger.debug("      task startCZDH："+startCZDH);
    					if(isInstanceStart.booleanValue() == true && currentCaoZuoDH.equals(startCZDH) ){
    						//启动流程的第一个节点 不发短信
    					}else{
    						//发送短息提醒给目标待办用户
    						try {
    							BusinessI busiObj = (BusinessI)sess.createCriteria(gn.getShiTiLei().getShiTiLeiClassName())
    									.add(Restrictions.eq("id",dataId))
    									.uniqueResult();
    							Object shiFouDXTZObj = PropertyUtils.getProperty(busiObj, "shiFouDXTZ");
    							if(shiFouDXTZObj!=null && shiFouDXTZObj instanceof Boolean){
    								Boolean shiFouDXTZ = (Boolean)shiFouDXTZObj;
    								if(shiFouDXTZ.booleanValue() == true){
//    									SMSUtils.sendMessage(gn,currentCaoZuoDH,busiObj,candidateUsers,yongHuMC,Calendar.getInstance().getTime());
    								}
    							}
    						} catch (Exception e) {
    							e.printStackTrace();
    					...
    					oim_Workflow_logger.debug("      task sendMessage end");
    				}else if("endEvent".equals(destinationType)){
    					
    					childLCJS = new LiuChengJS();
    					childLCJS.setGongNengDH(gongNengDH);
    					childLCJS.setGongNengObjId(dataId);
    					childLCJS.setShiFouGDJD(true);
    					childLCJS.setShiFouCL(true);
    					childLCJS.setShiFouWC(true);
    					childLCJS.setChuangJianRQ(Calendar.getInstance().getTime());
    					childLCJS.setCaoZuoRen(yongHuMC+" ");
    					
    					if("End".equals(transition.getDestination().getId())){
//    						setVariable("processIsEnded", true);
    						childLCJS.setCaoZuoDH("End");
        					childLCJS.setCaoZuoMC("结束");
    					}else if ("Terminate".equals(transition.getDestination().getId())){
//    						setVariable("processIsTerminated", true);
    						childLCJS.setCaoZuoDH("Terminate");
        					childLCJS.setCaoZuoMC("终止");
    					}
    					...
    					try {
	    					...
	    					if(shiFouDXTZObj!=null && shiFouDXTZObj instanceof Boolean){
	    						Boolean shiFouDXTZ = (Boolean)shiFouDXTZObj;
	    						if(shiFouDXTZ.booleanValue() == true){
	    							List<YongHu> users = new ArrayList<YongHu>();
	    							users.add(busiObj.getSuoYouZhe());
//	    							SMSUtils.sendMessage(gn,childLCJS.getCaoZuoDH(),busiObj,users,yongHuMC,Calendar.getInstance().getTime());
	    						}
	    					}
    					} catch (Exception e) {
							e.printStackTrace();
    				...
    				//父节点设置完成状态 并记录子节点信息
    				if(parentLCJS!=null){
    					if(!parentLCJS.getShiFouWC()){
    						parentLCJS.setCaoZuoRen(yongHuMC+" ");
    						parentLCJS.setCaoZuoXX("【已办理】");
    						parentLCJS.setShiFouWC(true);
    						parentLCJS.setShiFouCL(true);
    						parentLCJS.setShiFouCK(true);
    						parentLCJS.setHistoryId(historyId);
    						parentLCJS.setWanChengYH(new YongHu((Integer)getVariable("yongHuDM")));
                	...
                	oim_Workflow_logger.error("      创建流程检视信息失败",e);
    				e.printStackTrace();
    			}
    		}else{
    			oim_Workflow_logger.error("      task yongHuDM...is null");
//            	System.out.println("未绑定流程用户！");
    		}
    	}else{
    		oim_Workflow_logger.error("      task businessKey...is null");
//    		System.out.println("未绑定流程数据对象！");
    	}
    }else{
    	oim_Workflow_logger.error("      LiuChengJS...is not null");
    }
	oim_Workflow_logger.debug("   task transition ...完成");
